...
if "%key%"=="%content%" goto :preacceso

:error
cls
color 0c
cd "C:\Users\%username%\Desktop"
echo FECHA: %DATE%  HORA: %TIME%  CLAVE FALLIDA: "%key%">> history.intruder
attrib +s +h history.intruder
...
color 0c
echo.
echo. DETECTADA LA PRESENCIA DE UN ARCHIVO .intruder
echo. VERIFICAR LOS SIGUIENTES INTENTOS DE ACCESO A SCY-FILES.bat
echo.
attrib -s -h history.intruder
type history.intruder
del history.intruder
echo.
pause
...
cls
color 0c
echo. "%opc%" no es un valor valido, por favor intenta nuevamente
echo.
pause
echo.
goto:acceso
...
color 0c
cls
echo. Las claves deben de coincidir, intenta nuevamente.
pause
cls
echo.
goto :respass2
